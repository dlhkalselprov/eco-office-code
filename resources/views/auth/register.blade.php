        ...
        <div class="main-agileinfo">
            <div class="agileits-top">
                <form method="POST" action="{{ route('register') }}">
                    ...
                    <input required class="text" type="text" name="nama_instansi"
                        value="{{ old('nama_instansi') }}" placeholder="Nama Instansi/SKPD" />
                    @error('nama_instansi')
                        <h1 class="text-error" style="color:red"> {{ $message }}</h1>
                    @enderror

                    {{-- <input required class="text2" type="text" name="penanggung_jawab" value="{{ old('penanggung_jawab') }}"
                    placeholder="Nama Penanggung Jawab SKPD (Kepala)" >
                @error('penanggung_jawab')
                    <h1 class="text-error" style="color:red">  {{ $message }}</h1>
                @enderror --}}

                    <input required class="text2" type="text" inputmode="numeric" name="no_pic"
                        value="{{ old('no_pic') }}" placeholder="No Telp Narahubung SKPD (PIC)">
                    @error('no_pic')
                        <h1 class="text-error" style="color:red"> {{ $message }}</h1>
                    @enderror

                    <input required class="text2" type="text" name="nama_pic" placeholder="Nama Narahubung (PIC)"
                        value="{{ old('nama_pic') }}">
                    @error('nama_pic')
                        <h1 class="text-error" style="color:red"> {{ $message }}</h1>
                    @enderror

                    <input required class="text2" type="text" name="jabatan_pic" placeholder="Jabatan Narahubung"
                        value="{{ old('jabatan_pic') }}">
                    @error('jabatan_pic')
                        <h1 class="text-error" style="color:red"> {{ $message }}</h1>
                    @enderror

                    <input required class="text email" type="email" name="email" placeholder="Email SKPD"
                        value="{{ old('email') }}">
                    @error('email')
                        <h1 class="text-error" style="color:red"> {{ $message }}</h1>
                    @enderror
                    <div class="password-container">
                        <input required class="text2" id="password" type="password" name="password"
                    ...
                    @enderror
                    <div class="password-container">
                        <input required id="passwordconfirm" class="text w3lpass" type="password"
                            name="password_confirmation" placeholder="Konfirmasi Password">
                        <i class="fa fa-eye eye-icon" id="togglePasswordconfirm"></i>
                    </div>
